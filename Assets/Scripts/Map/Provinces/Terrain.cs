...
namespace Game
{
    ...
    #region Constructors
        ...
        public Terrain(Type type)
        {
            switch(type)
            {
                ...
                case Type.Forest:    Init(type, 1.00f, 1.25f, 0.75f, "Forest");      break;
                case Type.Hills:     Init(type, 0.75f, 1.50f, 0.75f, "Hills");       break;
                case Type.Jungle:    Init(type, 0.75f, 1.75f, 0.50f, "Jungle");      break;
                case Type.Mountains: Init(type, 0.50f, 2.00f, 0.25f, "Mountains");   break;
                case Type.Plains:    Init(type, 1.00f, 1.00f, 1.00f, "Plains");      break;
                case Type.Sea:       Init(type, 1.00f, 1.00f, 1.00f, "Sea");         break;
                case Type.Swamp:     Init(type, 0.75f, 1.25f, 0.50f, "Swamp");       break;
                case Type.Urban:     Init(type, 0.75f, 1.25f, 1.00f, "Urban");       break;
            }
        }
        ...
        {
            m_type = model.m_type;
            m_atkMod = model.m_atkMod;
            m_defMod = model.m_defMod;
    ...
    #region Functions
        ...
        // Init
        private void Init(Type type, float atk, float def, float mov, string fileName)
        {
            m_type = type;
        ...
        private void InitModifiers(float atk, float def, float mov)
        {
            m_atkMod = atk;
            m_defMod = def;
            m_movMod = mov;
